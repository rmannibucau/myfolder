...
module.exports = function() {
  var config = {
    ...
    output: {
      path: root('dist'),
      publicPath: '/', // ensure css will have right resource path
    ...
    resolve: {
      ...
      extensions: ['.ts', '.js', '.css', '.html', '.pug', '.less'],
      alias: {
        'app': 'src/app',
    ...
    module: {
      rules: [
        ...
        {test: /\.(eot|woff|woff2|ttf)$/, use: {loader: 'url-loader?limit=1024&name=style/image/[name].[hash].[ext]'}},
        {test: /\.(svg|png|jpg|gif)$/, use: {loader: 'url-loader?limit=1024&name=style/font/[name].[hash].[ext]'}}
      ],
